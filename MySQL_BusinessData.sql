...
-- from : to define table name
-- where : to define condition
-- order by : to sorting data
-- group by : it is used when aggregate functions are applied
-- having : to define condition when group by is used
...
-- alies : nickname
select max(creditLimit) as Max_Limit, min(creditLimit) as Min_Limit,
avg(creditLimit)
from customers;
...
where country="USA" and creditLimit > 20000;

select customername, country, creditLimit from customers
where country="USA" and creditLimit <= 20000;

select customername, country, creditLimit from customers
where country="USA" or creditLimit > 20000
order by country desc;

-- logical operator
-- and : return those records which matches all condition
-- or : returnn those records which matches any one condition
-- not : reverse the condition

select customername, country, creditLimit from customers
where not country='USA' order by country desc;
...
-- aggregate function: min, max, count, avg
select * from products;
select buyPrice from products;
desc products;
select count(*) from products;

-- display products in productLine
select productline, count(*) from products group by productline;
-- display min and max price of each product
...
select productLine, min(buyPrice) as Minimum_Price, max(buyPrice) as Maximum_Price
from products group by productLine;

-- display all details of products having minimun buyPrice
select * from products where buyPrice = (select min(buyPrice) from products);

update products
set buyPrice = 48.81
where productCode = 'S10_1678';

select * from products where buyPrice >= 30 and buyPrice <=50;
-- between operator: to define range
select * from products where buyPrice  between 30 and 50;

desc products;

-- display record of productCode, productName, quantityInStock, buyPrice, MSRP, totalBuyingPrice, totalSalePrice, profit_loss
select productCode, productName, quantityInStock, buyPrice, MSRP,
 quantityInStock * buyPrice as totalBuyingPrice, quantityInStock * MSRP as totalSalePrice,
 CASE 
        WHEN (quantityInStock * MSRP) > (quantityInStock * buyPrice) THEN 'Profit'
        ELSE 'Loss'
...
select productCode, productName, quantityInStock, buyPrice, MSRP,
(quantityInStock * buyPrice) as totalBuyingPrice, (quantityInStock * MSRP) as totalSalePrice,
(quantityInStock * MSRP - quantityInStock * buyPrice)  as profit_loss
from products;

select productCode, productName, quantityInStock, buyPrice, MSRP,
(quantityInStock * buyPrice) as totalBuyingPrice, (quantityInStock * MSRP) as totalSalePrice,
(quantityInStock * MSRP - quantityInStock * buyPrice)  as profit_loss
from products where (quantityInStock * MSRP - quantityInStock * buyPrice) > 10000;

select productCode, productName, quantityInStock, buyPrice, MSRP,
(quantityInStock * buyPrice) as totalBuyingPrice, (quantityInStock * MSRP) as totalSalePrice,
(quantityInStock * MSRP - quantityInStock * buyPrice)  as profit_loss
from products order by profit_loss desc 
limit 5;
...
-- and concept
select * from customers where creditLimit >= 10000 and creditLimit <= 50000;
-- same column creditLimit
select * from customers where creditLimit between 10000 and 50000;

select * from customers where country='UK' and creditLimit > 20000;
select * from customers where country='UK' or creditLimit > 20000;

select productline, sum(quantityinstock) as total_stock, count(*) as total_vehicle from products
group by productline
having total_stock > 30000
order by total_vehicle;

show tables;
select * from employees;
select * from offices;

select employeeNumber, concat(firstname, lastname) as empName,
email, city as officeLocation
from employees
inner join offices
on employees.officecode = offices.officecode
order by officeLocation;
...
-- inner join
select productName, productVendor, priceEach
from orderdetails
inner join  products
on orderdetails.productCode = products.productCode
order by productName;

select orderNumber, productName, priceEach
from orderdetails
inner join products
on orderdetails.productCode = products.productCode;

select orders.orderNumber, productName, priceEach, orderDate
from orderdetails
inner join products
on orderdetails.productCode = products.productCode
inner join orders
...
as employeeName,
e.jobtitle, concat(m.firstName,' ',m.lastName) as report_to,
m.jobtitle as reportPersonTitle
from employees e
...
use classicmodels;
select customers.customernumber,customername, sum(quantityordered*priceEach) as total
from orderdetails
inner join orders
on orderdetails.ordernumber = orders.ordernumber
...
-- bridgging
select customers.customerNumber, customerName,priceEach
from customers
inner join orders
...
-- subquery -It is a query nested within another query
-- subquery can be nested within another subquery
-- subquery is known as inner query and where as the query that contains subquery is called an outer query

-- display first and lastname of employees whose office code is 'USA'
select * from employees
...
select * from employees
where officeCode not in (select officeCode from offices where country = 'USA');

-- subquery with comparision
select * from payments;
...
-- display customers details who have not placed any order yet
select * from orders;
select * from customers;
select * from orderdetails;
...
-- corelated subquery
-- select products whose buy price are greater than avg buy price
...
-- exists
-- select customers whose one sale order with a value greater than 60k
...
select * from orders where customernumber in (148,259, 298);
use classicmodels;

-- case statement: allow to use conditional logic to specify results for various cases
...
case
    ...
    when creditlimit between 0 and 100000 then 'Low credit'
    when creditlimit between 100000 and 200000 then 'Average limit'
...
case
	when sum(amount) between 0 and 20000 then 'Low value'
    when sum(amount) between 20000 and 80000 then 'medium value'
    else 'High value'
end as remarks
from payments
group by customerNumber;

-- display customername with total ordercount by customer and define them with customer segmentation
with cte as (
select customernumber, customername, count(*) as orderCount from orders
inner join customers
using (customernumber)
group by customernumber
)
select customernumber, customername, orderCount,
case orderCount
...
select customernumber, customername, count(*) as orderCount,
case count(*)
	when 1 then 'One-time customer'
    when 2 then 'repeated customer'
    when 3 then 'frequent customer'
...
-- display total number of status on the basis of distince values using aggregate with case
...
select
    ...
    -- If function -> control flow functiion
...
-- ifnull(expr1, expr2): checking datacolumns if there is null values then show on that basis
...
add homePhone varchar(50);

insert into contacts
values
(1,'kaushal','98777777',Null),
(2,'Nikesh',Null, '9866666'),
(3,'Kusum', '11222222', '3455566'),
(4,'Jayram',null,'9844444')
;

select * from contacts;

select 
	contactName, phone, homephone
from contacts;

select contactname, 
	ifnull(phone,homephone) phonee
from contacts;

-- views-> it is same as returns table but it store as table
use classicmodels;
...
create view orders_orderdetils
as
...
select * from orders_orderdetils;

create view customerProduct
as
select customernumber,customername,productname,orderdate,country
from customers
inner join orders
using(customernumber)
inner join orderdetails
...
select customernumber, customername, concat(firstname, ' ',lastname)
as employeeName
from customers
inner join employees
-- using(salesrepemployeenumber)
on customers.salesrepemployeenumber = employees.employeenumber;

select * from customerpayments where customernumber=114;

-- update view of data only not columns or columns name, change take place in both view and table
...
drop view  orders_orderdetils;

select customernumber, sum(amount) from customerpayments
group by customernumber;

-- date function
select curdate(),current_date(),current_time(),
current_timestamp(),now(), utc_date(), utc_timestamp();
...
monthname(orderdate), day(orderdate), dayname(orderdate),
timestamp(orderdate), yearweek(orderdate), week(orderdate)
from orders;

select time(now()), hour(now()), minute(now()),
second(now());

select ordernumber,datediff(requiredDate, shippedDate)
from orders
where status='shipped';

select round(datediff(now(),'1999-02-18')/365);

select distinct(year(orderdate)) from orders;
select date_sub('1999-02-18', interval 1 year);
...
-- calculate total amount need to be collected per year on the basis of order
-- drop view q1;
-- drop view q2;
create view Q1
as
select year(orderDate) as ordered_year,sum(quantityOrdered * priceEach) as total_order_amount from orderdetails
inner join orders
using (orderNumber)
...
-- calculate total amount collected per year
create view q2
as
select year(orderDate) as collected_year,sum(amount) as total_collected  from payments
inner join orders
using (customerNumber)
group by year(orderDate);
...
-- how much amount still not collected
create view q3
as
select q1.ordered_year, q1.total_order_amount - q2.total_collected as amount_due
from q1
inner join q2
on q1.ordered_year = q2.collected_year;
...
create function getGrade(score float)
returns char(1)
deterministic
begin
...
delimiter //
create function getTotalPrice(quantity int, price float)
...
select quantityOrdered, priceEach, getTotalPrice(quantityOrdered, priceEach) as total
from orderdetails;
...
-- events -> used to schedule events

-- Check if the Event Scheduler is ON
...
drop event delete_particular_record;

SELECT * FROM orders WHERE DATE(orderDate) = '2003-01-06';
describe orders;

-- Create the event (problem due to foreign key)
CREATE EVENT delete_particular_record
ON SCHEDULE AT '2025-03-04 08:09:50'
DO
DELETE FROM orders WHERE DATE(orderDate) = '2003-01-06';
...
ON SCHEDULE EVERY 1 DAY STARTS '2025-03-01 00:00:00'
DO
...
-- stored procedure to generate and store otp

delimiter //
create procedure sp_generateOTP(in p_user_id int, out p_otp char(6))
begin
	set p_otp = generateOTP();
    set @expires_at = date_add(now(), interval 5 minute);
    replace into user_otps (user_id, otp, expires_at)
    values (p_user_id, p_otp, @expires_at);
end //
delimiter ;

-- usage of otp
call sp_generateOTP(1, @otp);
select @otp;

select * from user_otps;

-- stored procedure to verify the otp
delimiter //
create procedure sp_verifyOTP(
	in p_user_id int,
    in p_entered_otp char(6),
    ...
    begin
        ...
        select otp, expires_at into v_otp, v_expires_at
        from user_otps
        where user_id  = p_user_id;
        
        if v_otp is null then
        set p_result = "NO OTP found";
        elseif v_otp != p_entered_otp then
        set p_result = "Invalid OTP";
        elseif now() > v_expires_at then
        set p_result = "OTP expired";
        else
			set p_result = "OTP verified successfully";
            delete from user_otps where user_id = p_user_id;
		end if;
    end //
    delimiter ;
    
    call sp_verifyOTP(1, "524287", @result);
    select @result;
...
-- MySql Triggers
-- Trigger is a stored program invoked anutomatically
-- in response to an event such as insert, delete, or update
-- that occurs in associated table

-- why trigger
-- enforce business rules (e.g. prevent negative balance
-- in account)
--  audit changes (eg. log modifications to a history table)
-- ensure data integrity (eg. auto-correct or update related
-- records)
-- automate tasks (eg. sending notifications upon data changes
...
-- create a new table to store changes on the data of item
...
-- creating trigger for update item_tbl
delimiter //
...
for each row
begin
	insert into goods_change(id, change_type) values
    (new.id,"goods item updated");
end //
delimiter ;
...
('book', 3, 2000,12),
('samsung', 4, 2010,2),
('2003 Harley-Davidson Bike', 100, 2020, 2),
('2015 Bajaj Pulser', 50, 2021,4);
...
begin
    ...
    ' cannot  be 3 times greater than current quantity ',
    old.quantity);
...
-- before insert
-- it is used to validate data going  to be inserted.
...
begin
    ...
    set errorMessage = concat('Provided name ', new.name,
    ' is already present');
	if new.age < 0 then
		set new.age=0;
	end if;
...
drop trigger after_insert_students;
...
for each row
begin
	insert into student_log(std_id, action)
...
begin
	if old.id = 1 then 
    signal sqlstate '45000'
    set message_text = "can't delete id 1";
	end if;
end $$
delimiter ;
...
drop trigger after_delete_students;
